...
function getPolynom(...args) {
  const argsNumbers = args.slice().reverse();
  function polynom(x) {
    let result = 0;
    for (let i = 0; i < argsNumbers.length; i += 1) {
      result += argsNumbers[i] * x ** i;
    }
    return result;
  }
  return polynom;
...
function logger(func, logFunc) {
  return function a(...args) {
    const argsForLog = JSON.stringify(args).slice(1, -1);
    const stringForLog = `${func.name}(${argsForLog})`;
    logFunc(`${stringForLog} starts`);
    const result = func(...args);
    logFunc(`${stringForLog} ends`);
    return result;
  };
}
